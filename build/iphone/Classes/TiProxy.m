...
//Rare exceptions to indicate a bug in the _uiabroad code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_uiabroad = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_uiabroad]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.uiabroad.module"
